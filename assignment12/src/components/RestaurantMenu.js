        ...
        <ul className="flex flex-wrap">
          {Object.values(restaurant?.menu?.items)?.map((item) => (
            ...
            <img className="h-[50] object-cover"src={IMG_CDN_URL + item.cloudinaryImageId} />
              ...
              <h3 className="w[90]  text-orange-900 text-sm font-medium">{item.price/100}</h3>
              <button className="p-1 m-1 bg-green-600 font-semibold text-sm h-[40] text-white" onClick={() => addFoodItem(item)}>Add Item</button>
            </li>
          ))}
        </ul>
      </div>
